...
export default {
    ...
    methods: {
        getOrders(dataUrl = null){
            console.log(window.location.origin+`/api/agent/${this.agent}/ecommerce/orders/type/${this.parameter}`);
            axios.get(dataUrl ? dataUrl : window.location.origin+`/api/agent/${this.agent}/ecommerce/orders/type/delivered`).then(res=>{
                if (res.status == 200) {
